    ...
    */
    function renderCalendar() {
        let days = monthDetails.map((day, index)=> {
            return (
                // <div className={`${styles["calendar-day-container"]}` + (day.month !== 0 ? ' disabled' : '') + 
                //     (isCurrentDay(day) ? ' highlight' : '') + (isSelectedDay(day) ? ' highlight-green' : '')} key={index}>
                    
                <div className={`${styles["calendar-day-container"]} \+ (${((day.month) !== 0)} \? ' ${styles["disabled"]}' \: '') + 
                 (${isCurrentDay(day)} \? ' ${styles["highlight"]}' \: '') \+ (${isSelectedDay(day)} \? ' ${styles["highlight-green"]}' \: '')`}
                  key={index}>  

                  
                                                                                            
                    
                    <div className={`${styles["calendar-day-container__day"]}`}>
        ...
        return (
            <div className={styles.calendar__container}>
                <div className={styles.calendar__container__head}>
                    {weekdays.map((day,index)=><div key={index}
                     className={styles.calendar__container__name}>{day}</div>)}
                </div>
                <div className={styles.calendar__container__body}>
                    {days}
                </div>
            </div>
        )
    ...
    return (
        <div className={styles.datepicker}>
                ...
                <input className={styles.datepicker__input}
                 type='text' value={date.toLocaleDateString("en-US", options)}
                 readOnly /* onInput={props.handleResponse(date)} */
                 ref={inputRef}/>
                </div>
                {isComponentVisible ? (
                <div ref={ref} className={styles.datepicker__container}>
                    <div className={styles.datepicker__header}>
                            <div className={styles.datepicker__header__button}>
                                <div className={styles.datepicker__header__button__inner}
                                 onClick={()=> setNewYear(-10)}>
                                    <svg aria-hidden="true" focusable="false"
                                     data-prefix="fas" 
                                     data-icon="chevron-circle-left"
                                     className={`${styles["svg-inline--fa"]} ${styles["fa-chevron-circle-left"]} ${styles["fa-w-16"]}`}
                             ...
                             <div className={styles.datepicker__header__button}>
                                <div className={styles.datepicker__header__button__inner} 
                                  onClick={()=> setNewYear(-1)}>
                                    <svg aria-hidden="true" focusable="false"
                                     data-prefix="fas" data-icon="angle-double-left"                                     
                                     className={`${styles["svg-inline--fa"]} ${styles["fa-angle-double-left"]} ${styles["fa-w-14"]}`}
                                ...
                                <div className={styles.datepicker__header__button__inner} 
                                   onClick={()=> setNewMonth(-1)}>
                                    <svg aria-hidden="true" focusable="false" data-prefix="fas"
                                     ...
                                     className={`${styles["svg-inline--fa"]} ${styles["fa-angle-left"]} ${styles["fa-w-8"]}`}
                                     role="img" xmlns="http://www.w3.org/2000/svg"
                                      viewBox="0 0 256 512"><path fill="currentColor"
                                       d="M31.7 239l136-136c9.4-9.4 24.6-9.4 33.9 0l22.6 22.6c9.4 9.4 9.4 24.6 0 33.9L127.9 256l96.4 96.4c9.4 9.4 9.4 24.6 0 33.9L201.7 409c-9.4 9.4-24.6 9.4-33.9 0l-136-136c-9.5-9.4-9.5-24.6-.1-34z"></path></svg>
                                </div>
                            </div>
                            <div className={styles.datepicker__header__container}>
                                <div className={styles.datepicker__header__container__year}>{year}</div>
                                <div className={styles.datepicker__header__container__month}>{getMonthStr(month)}</div>
                            </div>
                             <div className={styles.datepicker__header__button}>
                                <div className={styles.datepicker__header__button__inner} onClick={()=> setNewMonth(1)}>
                                    <svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="angle-right"                                    
                                     className={`${styles["svg-inline--fa"]} ${styles["fa-angle-right"]} ${styles["fa-w-8"]}`}
                            ...
                            <div className={styles.datepicker__header__button} onClick={()=> setNewYear(1)}>
                                <div className={styles.datepicker__header__button__inner}>
                                    <svg aria-hidden="true" focusable="false" data-prefix="fas"
                                     data-icon="angle-double-right"                                     
                                      className={`${styles["svg-inline--fa"]} ${styles["fa-angle-double-right"]} ${styles["fa-w-14"]}`}
                                       role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
                                        <path fill="currentColor" d="M224.3 273l-136 136c-9.4 9.4-24.6 9.4-33.9 0l-22.6-22.6c-9.4-9.4-9.4-24.6 0-33.9l96.4-96.4-96.4-96.4c-9.4-9.4-9.4-24.6 0-33.9L54.3 103c9.4-9.4 24.6-9.4 33.9 0l136 136c9.5 9.4 9.5 24.6.1 34zm192-34l-136-136c-9.4-9.4-24.6-9.4-33.9 0l-22.6 22.6c-9.4 9.4-9.4 24.6 0 33.9l96.4 96.4-96.4 96.4c-9.4 9.4-9.4 24.6 0 33.9l22.6 22.6c9.4 9.4 24.6 9.4 33.9 0l136-136c9.4-9.2 9.4-24.4 0-33.8z"></path></svg>
                                </div>
                            </div>
                            <div className={styles.datepicker__header__button} onClick={()=> setNewYear(10)}>
                                <div className={styles.datepicker__header__button__inner}>
                                    <svg aria-hidden="true" focusable="false" data-prefix="fas"
                                     data-icon="chevron-circle-right"                                     
                                     className={`${styles["svg-inline--fa"]} ${styles["fa-chevron-circle-right"]} ${styles["fa-w-16"]}`}
